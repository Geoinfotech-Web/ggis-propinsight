...
def refresh_amenities(
    ...
    if not src_list:
        raise ValueError("at least one POI source must be configured")

    # Provider reads happen before the database transaction. A provider outage
    # cannot leave a half-written table, and other healthy providers can still
    # publish while the failed provider's existing rows remain untouched.
    ...
    # De-duplicate across providers as well as within each provider. Source
    # order is the precedence order for colliding named records.
    successful_sources = [source for source in src_list if source in fetched]
    merged = dedup_records(
        [record for source in successful_sources for record in fetched[source]]
    )
    publish_by_source = {source: [] for source in successful_sources}
    for record in merged:
        publish_by_source[record.source].append(record)

    # Replace successful providers and advance the registry atomically. Failed
    # providers are not replaced, so their last-known-good rows remain available.
    with connect() as conn:
        version = next_layer_version(conn, "poi")
        per_source = {
            source: replace_source_pois(conn, source, publish_by_source[source], version)
        ...
        published_version, invalidated = bump_layer(
            conn,
            "poi",
            source="+".join(successful_sources),
            notes=notes,
        )
        if published_version != version:  # defensive: both are in one transaction
            raise RuntimeError("POI layer version changed during publication")
