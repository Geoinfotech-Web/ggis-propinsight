...
# Tier-1 domains that can go live once their layers publish (Phase 1).
TIER1_REQUIRED_LAYERS: dict[str, tuple[str, ...]] = {
    "amenities": ("poi",),
    "accessibility": ("roads", "poi"),
    "feasibility": ("dem",),
}

# Tier 2–3 — always pending until a later phase wires them.
LATER_DOMAINS: tuple[str, ...] = ("security", "tenure", "market", "livability")
...
def pending_note(domain: str, versions: dict[str, str]) -> str:
    if domain in LATER_DOMAINS:
        return "Ships in a later phase (Tier 2-3)."
    required = TIER1_REQUIRED_LAYERS.get(domain, ())
...
def readiness_rows(versions: dict[str, str]) -> list[dict[str, object]]:
    ...
    for domain, required in TIER1_REQUIRED_LAYERS.items():
